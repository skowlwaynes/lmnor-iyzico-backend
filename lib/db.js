...
async function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS subtotal_kurus INTEGER`;
      await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS shipping_kurus INTEGER NOT NULL DEFAULT 0`;
      await sql`UPDATE orders SET subtotal_kurus = total_kurus - COALESCE(shipping_kurus, 0) WHERE subtotal_kurus IS NULL`;
