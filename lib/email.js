...
function getTrackingUrl(orderId) {
  ...
  baseUrl ||= "https://lmnor.framer.media";
...
function buildOrderConfirmation(order) {
  ...
  const total = formatTry(order.total_kurus);
      ...
      <div style="border:1px solid #252525;border-radius:18px;padding:32px;background:#080808;">
        ...
        <table role="presentation" style="width:100%;border-collapse:collapse;margin-bottom:18px;">
          ...
          <tr>
            ...
            <td style="padding-top:18px;color:#ffffff;font-weight:700;text-align:right;white-space:nowrap;">${total}</td>
          </tr>
        </table>
...
async function sendOrderConfirmation(order) {
  ...
  const from =
    process.env.RESEND_FROM || "LMNOR <onboarding@resend.dev>";
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    headers: {
      ...
      "Idempotency-Key": `lmnor-order-confirmation-${order.id}`,
    },
    body: JSON.stringify({
      from,
